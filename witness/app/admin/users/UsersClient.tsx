            ...
            <tbody>
              {users.map((user) => (
                ...
                >
                  ...
                  <td className="px-2 py-2 text-zinc-400">{user.last_login_at ? new Date(user.last_login_at).toLocaleString() : "-"}</td>
                </tr>
              ))}
              {!users.length && (
                <tr>
                  <td className="px-2 py-3 text-zinc-400" colSpan={6}>
        ...
        {selectedUser && (
          <div className="mt-4 space-y-4">
            <div className="rounded-lg border border-zinc-700 bg-zinc-950/70 p-3 text-sm text-zinc-200">
              <p className="font-semibold text-zinc-100">{selectedUser.display_name}</p>
              <p>{selectedUser.email}</p>
              <p className="mt-1 text-xs text-zinc-400">
                Orders: {selectedUser.order_count} · Submissions: {selectedUser.submission_count}
              </p>
            </div>
